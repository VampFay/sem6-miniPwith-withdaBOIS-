...
class Trainer:
    ...
    def train_epoch(self, epoch: int) -> EpochResult:
        ...
        for batch in progress:
            ...
            with self._autocast():
                loss = self.criterion(self.model(images), masks, distances).total
        ...
        return EpochResult(loss=float(np.mean(losses)))
    ...
    def validate(self) -> EpochResult:
        ...
        for batch in tqdm(self.loaders.validation, desc="validation"):
            ...
            losses.append(float(self.criterion(output, masks, distances).total))
    ...
    def fit(self) -> None:
        try:
            for epoch in range(self.start_epoch, self.config.epochs):
                ...
                if self.stale_epochs >= self.config.early_stopping_patience:
